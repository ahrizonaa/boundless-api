...
const client = new OAuth2Client(
	process.env.NIMBELWEAR_GOOGLE_CLIENT_ID,
	process.env.NIMBELWEAR_GOOGLE_CLIENT_SECRET,
...
async function VerifyGoogleUser(code) {
	try {
		let ticket = await client.verifyIdToken({
			idToken: code,
			audience: process.env.NIMBELWEAR_GOOGLE_CLIENT_ID
		});
...
function VerifyGoogleAudClientId(aud) {
	if (aud === process.env.NIMBELWEAR_GOOGLE_CLIENT_ID) {
		return true;
	}
	throw new Error(
...
async function SyncUserWithDB(googleprofile) {
	let user = await mongoClient.db('Storage').collection('Users').findOne({
