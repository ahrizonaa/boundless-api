...
app.get('/apps', async (req, res) => {
    let client = Utils.getClient()
    await client.connect()
    const allApps = await client.db("GirlCode").collection("Apps").find({}).toArray()
    res.send(allApps)
    client.close()
})
...
app.post('/features/save', async (req, res) => {
    ...
    let result = await client.db("GirlCode").collection("Apps").updateOne(query, update)
    res.send({
...
app.post('/features/delete', async (req, res) => {
    ...
    let result = await client.db("GirlCode").collection("Apps").updateOne(query, deleteElement)
    res.send({
        acknowledged: result.acknowledged,
        modifiedCount: result.modifiedCount
    })
